...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        current_pos, corner1, corner2, corner3, corner4 = state
        x,y = current_pos
        top, right = self.walls.height-2, self.walls.width-2
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                next_state = (next_pos, True if next_pos == (1,1) else corner1, True if next_pos == (1,top) else corner2, True if next_pos == (right, 1) else corner3, True if next_pos == (right, top) else corner4)
...
def cornersHeuristic(state, problem):
    ...
    location, corner1, corner2, corner3, corner4 = state
    corner_visited = [corner1,corner2,corner3,corner4]
    x, y = location
    maxx = 0
    minx = 0
    maxy = 0
    ...
    for (x,y) in foodList:
        ...
        if flag == 0:
            if y!=position[1] and y!=closestFood[1]:
                leftPoints = leftPoints + 1
        # if y!=position[1] and y!=closestFood[1] and x!=position[0] and x!=closestFood[0]:
        #     leftPoints += 1
...
def foodHeuristic(state, problem):
    ...
    for item in foodList:
        ...
        if yDistance > maxY:
            ...
            minY = yDistance
    heuristic1 = maxX - minX + maxY - minY
    
    #2nd heuristic=mazeDistance(my position, estimated closest food) + points left out(except those sharing row or col with these two pts)
    heuristic2 = foodHeuristic2(state,problem)
    
    heuristic3 = 0
    for food in foodList:
        distance = mazeDistance(position, food, problem.startingGameState)
        if distance > heuristic3:
            heuristic3 = distance

    return max([heuristic1,heuristic2,heuristic3])  
